...
def fetch_all_incidents(db_config):
    ...
    try:
        ...
        cursor = conn.cursor()  
        cursor.execute("SELECT inc_id, short_description, description, sys_created_on, open_since, state, run_status, priority, cmdb_ci FROM (SELECT t2.inc_id, t2.short_description, t2.description, t2.raised_date AS sys_created_on, t2.raised_date AS open_since, t2.state, t1.run_status, t2.priority, t2.cmdb_ci, ROW_NUMBER() OVER (PARTITION BY t2.inc_id ORDER BY CASE t1.run_status WHEN 'Processing Updates' THEN 0 WHEN 'CI Unavailable' THEN 1 WHEN 'Incident Processed' THEN 2 WHEN 'Incident Processing' THEN 3 WHEN 'Incident Received' THEN 4 ELSE 5 END ) AS rn FROM agent_run_status t1 LEFT JOIN p1p2_incidents t2 ON t1.incident_id = t2.inc_id) ranked WHERE rn = 1 ORDER BY sys_created_on desc;")
